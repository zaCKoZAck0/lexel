...
import { Key, Eye, EyeOff, Loader2, Star, Trash2, ExternalLink } from 'lucide-react'
...
interface ApiKeysListProps { keys: ApiKey[] }
    ...
    return (
        ...
        <div className="space-y-6">
            {Object.entries(groupedKeys).map(([providerId, providerKeys]) => (
                <ProviderCard key={providerId} providerId={providerId} keys={providerKeys} />
            ))}
        </div>
    )
}
    ...
    return (
        <Card>
            <CardHeader className="text-center pb-2">
                ...
                <CardDescription>Add your first API key to start using AI models in your projects.</CardDescription>
            </CardHeader>
...
interface ProviderCardProps { providerId: string; keys: ApiKey[] }
        ...
        <Card>
            <CardHeader>
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="text-2xl">{Icon && <Icon className="size-8" />}</div>
                        <div>
            ...
            <CardContent>
                <div className="space-y-4">
                    {keys.map(key => (
                        <ApiKeyRow key={key.id} apiKey={key} providerName={providerInfo?.name} />
                    ))}
                </div>
            </CardContent>
        </Card>
...
interface ApiKeyRowProps { apiKey: ApiKey; providerName?: string }
...
function ApiKeyRow({ apiKey, providerName }: ApiKeyRowProps) {
    ...
    const deleteMutation = useMutation({
        ...
        onMutate: async (id) => {
            ...
            if (previousKeys) {
                queryClient.setQueryData(apiKeyQueryKeys.all, previousKeys.filter(k => k.id !== id))
            }
            return { previousKeys }
        },
        onError: (error, _id, ctx) => {
            if (ctx?.previousKeys) queryClient.setQueryData(apiKeyQueryKeys.all, ctx.previousKeys)
            const msg = error instanceof ApiRequestError ? (error.body?.message || error.statusText) : 'Failed to delete API key'
            toast.error(msg)
        },
        onSuccess: () => { toast.success('Deleted — key gone') }
    })
    ...
    const setDefaultMutation = useMutation({
        ...
        onMutate: async (id) => {
            ...
            const previousKeys = queryClient.getQueryData<ApiKey[]>(apiKeyQueryKeys.all)
            if (previousKeys) {
                queryClient.setQueryData(apiKeyQueryKeys.all, previousKeys.map(k => ({ ...k, default: k.id === id, updatedAt: new Date() })))
            }
            return { previousKeys }
        },
        onError: (error, _id, ctx) => {
            if (ctx?.previousKeys) queryClient.setQueryData(apiKeyQueryKeys.all, ctx.previousKeys)
            const msg = error instanceof ApiRequestError ? (error.body?.message || error.statusText) : 'Failed to set default API key'
            toast.error(msg)
        },
        onSuccess: () => {
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between p-4 border rounded-lg">
            ...
            <div className="flex-1 space-y-2 min-w-0">
                ...
                <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    {(() => {
                        ...
                        const isUpdated = Math.abs(updatedAt.getTime() - createdAt.getTime()) > 60_000
                        const refTime = isUpdated ? updatedAt : createdAt
                        return (
                            <span>
                                {isUpdated ? 'Updated' : 'Added'} {formatDistanceToNow(refTime, { addSuffix: true })}
                            </span>
                        )
                    })()}
                </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
                <Tooltip>
                    <TooltipTrigger asChild>
                        <span>
                            <Button
                                ...
                                size="sm"
                                onClick={() => !apiKey.default && setDefaultMutation.mutate(apiKey.id)}
                                disabled={apiKey.default || setDefaultMutation.isPending || isMutating}
                                aria-label={apiKey.default ? 'Current default key' : 'Make this key default'}
                            >
                                {setDefaultMutation.isPending && setDefaultMutation.variables === apiKey.id ? (
                                    <Loader2 className="h-4 w-4 animate-spin" />
                                ) : (
                                    <>
                                        {/* Increase icon/text separation to 8px to align with spacing scale */}
                                        <Star className={"h-4 w-4 mr-2 " + (apiKey.default ? 'fill-current' : '')} />
                                        Default
                                    </>
                                )}
                            </Button>
                    ...
                    <TooltipContent>{apiKey.default ? 'This is the default key' : 'Set as default'}</TooltipContent>
                </Tooltip>
                    ...
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <AlertDialogTrigger asChild>
                                <Button disabled={isMutating} variant="outline" size="sm" aria-label="Delete API key">
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                                ...
                                {apiKey.default && (
                                    <span className="block mt-2 font-medium text-orange-600">This is your default key for {providerName}.</span>
                                )}
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                            <AlertDialogCancel>Cancel</AlertDialogCancel>
                            <AlertDialogAction onClick={() => deleteMutation.mutate(apiKey.id)} disabled={deleteMutation.isPending || isMutating} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
