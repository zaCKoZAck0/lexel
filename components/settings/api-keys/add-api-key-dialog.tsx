...
import { CreateApiKeyInput, ApiKey, apiKeyQueryKeys } from '@/lib/types/api-keys'
...
export function AddApiKeyDialog({ disabled = false }: { disabled?: boolean }) {
    ...
    const createMutation = useMutation({
        ...
        onMutate: async (newKey) => {
            ...
            if (previousKeys) {
                ...
                queryClient.setQueryData(apiKeyQueryKeys.all, [...previousKeys, optimisticKey])
            }
            return { previousKeys }
        },
        onError: (error, _vars, ctx) => {
            ...
            const msg = error instanceof ApiRequestError ? (error.body?.message || error.statusText) : 'Failed to create API key'
            toast.error(msg)
        },
        onSuccess: () => {
    ...
    const isPatternValid = selectedProvider && newKeyValue
    ...
    const handleCreateKey = async () => {
        ...
        const keyData: CreateApiKeyInput = { provider: selectedProvider, key: newKeyValue.trim(), default: false }
            ...
            <DialogContent className="sm:max-w-md space-y-5">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-sm leading-relaxed max-w-prose">
                        Add an API key for your preferred AI provider. Stored encrypted & only used server-side.
                    </DialogDescription>
                </DialogHeader>
                <div className="grid gap-5">
                    ...
                    <div className="grid gap-2">
                        ...
                        <div className="flex items-center gap-2">
                            <Input id="api-key" placeholder="Enter your API key" type={showKey ? 'text' : 'password'} value={newKeyValue} onChange={(e) => setNewKeyValue(e.target.value)} className="flex-1" />
                            <Tooltip>
                ...
                <DialogFooter className="gap-2">
                    ...
                    <Button onClick={handleCreateKey} disabled={createMutation.isPending || !selectedProvider || !newKeyValue.trim() || !isPatternValid}>
