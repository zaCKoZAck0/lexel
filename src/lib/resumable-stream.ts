...
export function createResumableStreamContext(options: Options): ResumableStreamContext {
    ...
    return {
        async resumableStream(streamId, makeStream, skipChars = 0) {
            ...
            const currentCount = await redis.incr(sentinelKey).catch((err) => {
                if (String(err).includes("ERR value is not an integer")) return STREAM_DONE;
                throw err;
            });
