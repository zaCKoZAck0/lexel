...
const updateSchema = z.object({
    ...
    if (data.provider && data.key) {
        ...
        if (info?.keyPattern && data.key && !info.keyPattern.test(data.key.trim())) {
            ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['key'], message: 'Key format invalid for provider' })
        }
    }
});
...
export async function PATCH(
    ...
    try {
        ...
        const { id } = await params;
        const updatedKey = await apiKeysService.updateById(id, session.user.id, parsed);
        ...
        if (err instanceof Error && err.message === 'API key already exists for this user') {
