...
export function WebSearchResults({
  ...
  return (
    <Accordion type="single" collapsible className={className}>
      <AccordionItem value="item-1" className="data-[state=closed]:hover:bg-muted rounded-md border last:border-b transition-colors">
        ...
        <AccordionContent className="px-2 pb-1">
          <ScrollArea className="h-[200px] pr-2">
            {formatted.map(r => (
              <a href={r.url} key={r.key} className="hover:bg-muted rounded-md transition-colors block px-1 py-1.5 w-full">
                <div className="flex items-center gap-3 min-w-0">
                  {r.favicon && <img src={r.favicon} alt={r.title} className="w-4 h-4 shrink-0" />}
                  <p className="flex items-center gap-2 min-w-0 w-full">
                    <span className="flex-1 truncate">{r.title}</span>
                    <span className="text-xs text-muted-foreground pl-2 shrink-0 max-w-[40%] truncate">{r.site}</span>
                  </p>
                </div>
              </a>
            ))}
