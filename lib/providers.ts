...
import { OpenAIIcon, AnthropicIcon, GeminiIcon, DeepSeekIcon, QwenIcon } from '@/components/icons/provider-icons'
...
export interface ProviderConfig {
    ...
    /** Representative / commonly used models to show in UI pickers. */
    models: string[]
...
export const PROVIDER_MAP: Record<ProviderId, ProviderConfig> = Object.freeze(
    AI_PROVIDERS.reduce((acc, p) => { (acc as any)[p.id] = p; return acc }, {} as Record<ProviderId, ProviderConfig>)
)

export function getProviderInfo<T extends string>(providerId: T): T extends ProviderId ? ProviderConfig : ProviderConfig | undefined
export function getProviderInfo(providerId: string) {
    return (PROVIDER_MAP as Record<string, ProviderConfig | undefined>)[providerId]
}

export function validateProviderKey(providerId: ProviderId, key: string): boolean {
    ...
    return prefix ? key.startsWith(prefix.replace(/\s+/g, '')) : key.trim().length > 0
}
