...
export async function create(
  ...
  const keyCountForProvider = await prisma.apiKeys.count({
    where: { userId: data.userId, provider: data.provider },
  });

  if (keyCountForProvider >= MAX_API_KEYS_PER_PROVIDER) {
    throw new AppError(
      'Maximum number of API keys reached for this provider',
      409,
    );
  }
...
export async function findAllByUserId(
  ...
  const where: { userId: string; default?: boolean } = { userId };

  if (filters?.default !== undefined) {
    where.default = filters.default;
  }

  const records = await prisma.apiKeys.findMany({ where });
...
export async function setDefault(id: string, userId: string) {
  ...
  if (key.userId !== userId) {
    throw new AppError(
      'Unauthorized: You can only modify your own API keys',
      403,
    );
  }
...
export async function getDefaultApiKeyForProvider({
  ...
  const record = await prisma.apiKeys.findFirst({
    where: { userId, provider, default: true },
  });
...
export async function deleteById(id: string, userId: string) {
  ...
  if (key.userId !== userId) {
    throw new AppError(
      'Unauthorized: You can only delete your own API keys',
      403,
    );
  }
...
export async function findById(id: string, userId: string) {
  ...
  if (key.userId !== userId) {
    throw new AppError(
      'Unauthorized: You can only access your own API keys',
      403,
    );
  }
...
export async function updateById(
  ...
  if (existingKey.userId !== userId) {
    throw new AppError(
      'Unauthorized: You can only update your own API keys',
      403,
    );
  }
