...
import { QueryKey, QueryFunction, QueryClient } from '@tanstack/react-query';
...
export function createQueryFunction<TData>(
  ...
  return async () => {
    try {
      ...
      throw new ApiRequestError(0, 'Unknown Error', {
        message:
 ...
 */
export function createOptimisticHelpers<
...
>() {
  return {
    ...
    addOptimistic: (items: TData[], newItem: TData): TData[] => [
      ...items,
      { ...newItem, id: `temp-${Date.now()}` as string },
    ],
    ...
    replaceTemporaryId: (
      ...
      serverItem: TData,
    ): TData[] => items.map(item => (item.id === tempId ? serverItem : item)),
  };
}
